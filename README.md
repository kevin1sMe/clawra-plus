...
| `google` | `generate` / `edit` | `gemini-3-pro-image-preview` / `gemini-3-pro-image-preview`（支持 `gemini-3.1-flash-image`、`nano-banana-2`、`gemini-2.5-flash-image`） | `GOOGLE_API_KEY` |
...
GOOGLE_EDIT_IMAGE_URL=https://example.com/input.png  # 可选，google 编辑参考图 URL
GOOGLE_EDIT_IMAGE_PATH=/path/to/input.png            # 可选，优先于 URL
...
DEFAULT_MODEL_GOOGLE_GENERATE=gemini-3.1-flash-image
DEFAULT_MODEL_GOOGLE_EDIT=gemini-3.1-flash-image
